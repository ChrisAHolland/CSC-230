...
; *********************************************
	...
	;ldi r16, ' '
	;sts DOTDASH_PATTERN, r16
	;sts DOTDASH_PATTERN+1, r16
	;sts DOTDASH_PATTERN+2, r16
	;sts DOTDASH_PATTERN+3, r16
	;sts DOTDASH_PATTERN+4, r16
	;sts DOTDASH_PATTERN+5, r16
	;sts DOTDASH_PATTERN+6, r16
	;sts DOTDASH_PATTERN+7, r16
	;sts DOTDASH_PATTERN+8, r16
	;sts DOTDASH_PATTERN+9, r16
	
	ldi r16, 0
	sts BUTTON_LENGTH, r16
	
	; Load the dotdah_pattern into XH:XL to use as a global variable
	ldi XH, high(DOTDASH_PATTERN)
	ldi XL, low(DOTDASH_PATTERN)
	; Counter for part4
	ldi r16, 0
	sts PATTERN_LENGTH, r16
	
	; def's for the check button routine
	.def DATAH=r27  
	.def DATAL=r26	
...
; Returning after interrupt handler (modelled from lab8)
	timer_1_off:
		pop r16
		sts SREG, r16
		pop r17
...
timer4:
	...
	; Load the amount of times the button was pushed
	lds YH, BUTTON_COUNT
	...
	breq potential_add
	cpi r24, 0
	breq pattern_initiate

	potential_add:
		; Checks if previous cycle had a button press(Part 3)
		lds r17, BUTTON_PREVIOUS
		cpi r17, 1
		brne button_on
		
		; Increments the button length
		lds r16, BUTTON_LENGTH
		inc r16
		sts BUTTON_LENGTH, r16
		rjmp timer4_finished
		
	; Adds one to the button count
	button_on:
		; Resets button-length to 1
		ldi r16, 1
		sts BUTTON_LENGTH, r16
		
		; Adds one to button_count
		adiw YH:YL, 1
		sts BUTTON_COUNT, YH
		sts BUTTON_COUNT + 1, YL
		rjmp timer4_finished
	
	; Checks which value should be added to the pattern
	pattern_initiate:
		; Skip adding to the pattern if we already have 10
		lds r16, PATTERN_LENGTH
		cpi r16, 10
		breq timer4_finished
		
		; Initial case to make sure the length isn't 0
		lds r16, BUTTON_LENGTH
		tst r16
		breq timer4_finished
		
		; Add a dash if length is >= 200 milliseconds
		cpi r16, 20
		brge add_dash
	
	; Adds a dot if button length is less than 20 (200ms/10ms/cycle)
	add_dot:                                               
		ldi r16, '.'
		st X+, r16
		sts DOTDASH_PATTERN, r16
	...
	; Adds a dash if the length is >= 20
	add_dash:
		ldi r16, '-'
		st X+, r16
		sts DOTDASH_PATTERN, r16
